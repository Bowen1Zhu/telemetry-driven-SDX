...
LOGGER = logging.getLogger("sdx_general_validate")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate generalized multi-flow SDX steering")
    parser.add_argument(
        "--config",
        default=os.path.join(REPOSITORY_DIRECTORY, "config/run_config_generalized.json"),
        help="Path to the generalized json config file",
    )
    parser.add_argument(
        "--results-dir",
        default=os.path.join(REPOSITORY_DIRECTORY, "results/generalized_validation"),
        help="Directory for csv and json results",
    )
    parser.add_argument("--warmup-s", type=float, default=18.0, help="Seconds to wait for FRR/BGP warm-up")
    parser.add_argument("--fast-max-ms", type=float, default=120.0, help="Pass threshold for fast-path RTT")
    parser.add_argument("--slow-min-ms", type=float, default=500.0, help="Pass threshold for slow-path RTT")
    return parser.parse_args()


class GeneralizedValidationRunner:
    def __init__(
        self,
        network: Mininet,
    ...
    async def apply_assignments(self, assignments: dict[str, str]) -> None:
        await asyncio.gather(
            *(self.controller.set_group_path(group_name, path_name) for group_name, path_name in assignments.items())
        )
    ...
    async def run_validation(self) -> None:
        for scenario in self.config.validation_scenarios:
            ...
            for session, result in zip(self.config.traffic_sessions, results):
                expected_path = scenario.assignments.get(
                    session.forward_group,
                    self.controller.group_current_path.get(session.forward_group, "unknown"),
                )
                ...
                LOGGER.info(
                    "%s / %s expected=%s avg_ms=%s loss_pct=%s pass=%s",
                    scenario.name,
                    session.name,
                    expected_path,
                    avg_ms,
    ...
    def write_summary(self) -> None:
        ...
        for scenario in self.config.validation_scenarios:
            scenario_rows = [row for row in self.rows if row["scenario"] == scenario.name]
            pass_count = sum(1 for row in scenario_rows if row["pass"])
            scenarios[scenario.name] = {
                "description": scenario.description,
                "total_sessions": len(scenario_rows),
                "pass_count": pass_count,
                "fail_count": len(scenario_rows) - pass_count,
                "avg_rtt_ms": (statistics.mean(float(row["avg_ms"]) for row in scenario_rows if row["avg_ms"] is not None) if scenario_rows else None),
            }
...
async def async_main(args: argparse.Namespace, network: Mininet, config: RunConfig) -> None:
    ...
    runner = GeneralizedValidationRunner(
        network=network,
        controller=controller,
        config=config,
        results_dir=args.results_dir,
        fast_max_ms=args.fast_max_ms,
        slow_min_ms=args.slow_min_ms,
    )
    try:

...
class Topology(Topo):
    """Larger BGP-aware topology with 4 IXPs and a multi-stage loop-capable transit core.

    Physical picture:
      AS100/AS500 -- IXP1 -- {slow12, fast12} -- IXP2(shared fabric)
        -- {slow23, fast23} -- IXP3(shared fabric) -- {slow34, fast34} -- IXP4 -- AS200/AS600

    The two shared middle IXPs create real cycles in the AS-level peering graph.
    The SDX still exposes two end-to-end legal path classes for steering:
      * slow = slow12 -> slow23 -> slow34
      * fast = fast12 -> fast23 -> fast34
    """
    ...
    def build(self, *args, **params):
        ...
        # IXPs: outer two are P4 steering points, middle two are shared fabrics
        ...
        ixp3s1 = self.addSwitch('ixp3s1', cls=OVSSwitch)
        ...
        # IXP1: AS100, AS500, slow12, fast12
        ...
        # IXP2 shared middle fabric: stage1->stage2
        ...
        # IXP3 shared middle fabric: stage2->stage3
        ...
        # IXP4: AS200, AS600, slow34, fast34
